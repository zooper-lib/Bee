# railway-guard-conditions

## Purpose

Enable conditional guard groups in the railway DSL so guards can be gated by a condition over the request, with support for nesting (composed via logical AND) and a unified `When` operator that supersedes the deprecated `Branch` alias.
